...
bc(javascript). Binds : ['update', 'include', ...]  // yadi yada
...
bc(javascript).. Class.Mutators.BindAll = function(self, bool) {
...
p. With this, all methods get bound, and it's this simple:

bc(javascript). BindAll : true  // or anything else truthy

So now, class methods can be passed around without fear of them forgetting who @this@ is:

bc(javascript). $('flashing_red').addEvent('click', self.destruct);
